...
public class MainCLI {
     ...
     * serializzate tramite JsonUtil. In caso di errore il sistema parte da una
     * sessione vuota.
     */
    ...
    /**
     * Esegue il loop applicativo principale. Ad ogni iterazione controlla le
     * scadenze
    ...
    private void loopLogin() {
        ...
        if (gestoreSessione.login(nomeUtente, password)) {
            ...
            if (vista.leggiBooleano("Vuoi registrarti come nuovo Fruitore con questo nome utente?")) {
                ...
                gestoreSessione.creaFruitore(nomeUtente, nuovaPass);
     ...
     * Permette al Configuratore di caricare in massa categorie e proposte da file
     * testuale.
     */
    ...
    /**
     * Gestisce la procedura di ritiro di una singola iscrizione da parte del
     * Fruitore,
    ...
    private void creaProposta() {
        ...
        Proposta proposta = gestoreProposte.getBacheca().creaProposta(categoria);
    ...
    private void creaCategoria() {
        ...
        try {
            Categoria nuovaCategoria = gestoreCategorie.creaCategoria(nome, descrizione, nomePadre);
            ...
            while (aggiuntaCampi) {
                if (vista.leggiBooleano("Vuoi aggiungere un campo specifico (per questa categoria)?")) {
                    ...
                    try {
                        TipoCampo tipo = TipoCampo.values()[Math.min(Math.max(0, idxTipo),
                                TipoCampo.values().length - 1)];
                        nuovaCategoria.creaCampo(nomeCampo, descCampo, obbligatorio, tipo);
                    } catch (Exception e) {
    ...
    private void aggiungiCampoComune() {
        ...
        try {
            gestoreCategorie.creaCampoComune(nomeCampo, descCampo, obbligatorio, tipo);
